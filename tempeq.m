% TEMPORARY EQUILIBRIUM (INNER LOOP)
...
pi_seq_res=zeros(50,50,4,T+1);   % T+1 because of initial pi0
...
pi_seq_res([1:50],[1:50],[1:4],1)=pi0_res([1:50],[1:50],[1:4]);
...
for j=1:4
    for n=1:50
        for t=1:T
            P_dot_res(j,n,t)=( sum(pi0_res(:,n,j).*x_dot_res(j,:).^(-theta(j)) ) )^(1/theta(j));
        end
    end
end
...
for n=1:50
    for i=1:50
        for t=1:T
            pi_seq_res(n,i,:,t+1)=pi_seq_res(n,i,:,t).*( x_dot_res(:,i,t) ./ P_dot_res(:,n,t) ).^(-theta(j));
        end
    end
end
...
for n=1:50
    for k=1:4
        for t=1:T+1
            sum_piX(k,n,t)=sum(pi_seq_res(n,:,k,t).*X_seq_res(k,:,t));
        end
    end
end
